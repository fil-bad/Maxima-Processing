...
void draw() 
{
  ...
  fill(255, 0, 0);
  text("giunto = " + str(giunto+4) + " DIRETTA", 10, 20); 
...
void r3_6Calc()
{
  ...
  R3_6[0][1]=  sin(theta[0])*cos(theta[1])*cos(theta[2]) + cos(theta[0])*sin(theta[2]);
  R3_6[0][2]= -sin(theta[1])*cos(theta[2]);
  R3_6[0][3]=0.0;
  /* Calc Scorrimento */
...
void RPY_Calc(boolean sol)
{
  ...
  {
    RPY[1]=atan2(-R3_6[0][2], +sqrt(sq(R3_6[1][2])+sq(R3_6[2][2])));  //pitch
  } else { //soluzione negativa
    RPY[1]=atan2(-R3_6[0][2], -sqrt(sq(R3_6[1][2])+sq(R3_6[2][2])));  //pitch
  }
  println("-R3_6[0][2]:"+-R3_6[0][2]+" R3_6[1][2]:"+R3_6[1][2]+" R3_6[2][2]"+R3_6[2][2]);

  if (RPY[1]==PI/2)
  {
    RPY[2]=0;  //yaw arbitrario
    RPY[0]=atan2(R3_6[1][2], R3_6[2][2]); //roll
    return;
  } else if (RPY[1]==-PI/2)
  {
    RPY[2]=0;  //yaw arbitrario
    ...
    RPY[2]=atan2(R3_6[1][2]/cos(RPY[1]), R3_6[2][2]/cos(RPY[1]));  //yaw 
    RPY[0]=atan2(R3_6[0][1]/cos(RPY[1]), R3_6[0][0]/cos(RPY[1]));  //roll
  }
}
