...
void draw() 
{
  ...
  //todo: visualizzare matrice di posizione desiderata 
  //rispetto alla terna (x3,y3,z3).
  //se cinematica diretta calcolarla dati gli angoli
  //se cinematica inversa modificare lei e calcolare gli angoli (visualizzare solizione scelta (angoli diversi, posa uguale))


  //todo: calcolare dati i thetha la posizione nello spazio di lavoro della pinza
  //in X,Y,Z rispetto sempre a (x3,y3,z3)
...
{
  /* Calc Normale */
  R3_6[0][0]=cos(theta[0])*cos(theta[1])*cos(theta[2])-sin(theta[0])*sin(theta[2]);
  R3_6[0][1]=sin(theta[0])*cos(theta[1])*cos(theta[2])+sin(theta[0])*sin(theta[2]);
  R3_6[0][2]=-sin(theta[1])*sin(theta[2]);
  R3_6[0][3]=0.0;
  /* Calc Scorrimento */
  R3_6[1][0]=-cos(theta[0])*cos(theta[1])*sin(theta[2])-sin(theta[0])*cos(theta[2]);
  R3_6[1][1]=-sin(theta[0])*cos(theta[1])*sin(theta[2])+cos(theta[0])*cos(theta[2]);
  R3_6[1][2]=sin(theta[1])*sin(theta[2]);
  R3_6[1][3]=0.0;
  /* Calc Avanzamento */
  R3_6[2][0]=-cos(theta[0])*sin(theta[1]);
  R3_6[2][1]=-sin(theta[0])*sin(theta[1]);
  R3_6[2][2]=cos(theta[1]);
  R3_6[2][3]=0.0;
  /* Calc Pos */
...
void RPY_Calc(boolean sol)
{
  ...
  {
    RPY[1]=atan2(-R3_6[2][0], +sqrt(sq(R3_6[2][1])+sq(R3_6[2][2])));  //pitch
    if (RPY[1]==PI/2)
    {
      RPY[2]=0;  //yaw arbitrario
      RPY[0]=atan2(R3_6[2][2], R3_6[0][2]); //roll
      return;
    }
    if (RPY[1]==-PI/2)
    {
    ...
    RPY[2]=atan2(R3_6[2][1]/cos(RPY[1]), R3_6[2][2]/cos(RPY[1]));  //yaw 
    RPY[0]=atan2(R3_6[1][0]/cos(RPY[1]), R3_6[0][0]/cos(RPY[1]));  //roll
  } else { //sol negativa
    RPY[1]=atan2(-R3_6[2][0], -sqrt(sq(R3_6[2][1])+sq(R3_6[2][2])));  //pitch
    if (RPY[1]==PI/2)
    {
